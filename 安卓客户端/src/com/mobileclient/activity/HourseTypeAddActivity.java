...
public class HourseTypeAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.hoursetype_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加房屋类别");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_typeName.getText().toString().equals("")) {
						Toast.makeText(HourseTypeAddActivity.this, "房屋类型输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传房屋类别信息*/
					HourseTypeAddActivity.this.setTitle("正在上传房屋类别信息，稍等...");
					String result = hourseTypeService.AddHourseType(hourseType);
